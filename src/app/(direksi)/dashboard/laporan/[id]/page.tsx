  ...
  if (!report) {
    return (
      <div className="p-6 md:p-10">
        <Link href="/dashboard/laporan" className="inline-flex items-center text-sm text-slate-400 hover:text-white mb-6">
          <ArrowLeft className="w-4 h-4 mr-2" />
          Kembali ke Daftar Laporan
        </Link>
        <div className="p-8 text-center bg-slate-900 border border-slate-800 rounded-xl">
          <FileText className="w-12 h-12 text-slate-500 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-slate-300">Laporan Tidak Ditemukan</h2>
          <p className="text-slate-500 mt-2">Laporan yang Anda cari mungkin sudah dihapus atau tidak tersedia.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="p-6 md:p-10 max-w-4xl">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="p-4 bg-slate-900/50 border border-slate-800 rounded-xl flex items-start gap-4">
          ...
          <div>
            <p className="text-sm text-slate-400">Tanggal Laporan</p>
      ...
      <div className="space-y-6 mb-8">
        <h3 className="text-xl font-bold border-b border-slate-800 pb-2">Rincian Tugas & Capaian</h3>
        
        {(!report.task_updates || report.task_updates.length === 0) ? (
          <div className="p-4 text-center text-slate-500 bg-slate-900/20 rounded-xl border border-slate-800">
            ...
            {report.task_updates.map((update: any) => (
              <div key={update.id} className="p-4 rounded-xl border border-slate-800 bg-slate-900/50 flex flex-col md:flex-row gap-4">
                <div className="flex-1">
                  ...
                  {update.notes ? (
                    <p className="text-sm text-slate-400 mt-2 bg-slate-950 p-2 rounded-md border border-slate-800">
                      <span className="text-xs text-slate-500 block mb-1">Catatan Staff:</span>
                      {update.notes}
                    </p>
                  ) : (
                    <p className="text-sm text-slate-500 mt-1 italic">Tidak ada catatan.</p>
                  )}
                </div>
      ...
      {report.evidence_url && (
        <div className="mb-8 p-6 bg-slate-900/50 border border-slate-800 rounded-xl">
          <h3 className="text-xl font-bold border-b border-slate-800 pb-2 mb-4 flex items-center gap-2">
            <ImageIcon className="w-5 h-5 text-emerald-400" />
            Bukti Foto Laporan
          </h3>
          <div className="mt-4">
            <a href={report.evidence_url} target="_blank" rel="noreferrer" className="block w-full max-w-lg overflow-hidden rounded-xl border border-slate-700 hover:border-emerald-500 transition-colors">
              <img 
            ...
            <p className="text-slate-500 text-sm mt-2">Klik gambar untuk melihat resolusi penuh</p>
          </div>
        </div>
      )}
